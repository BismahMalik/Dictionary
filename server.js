...
//     const word = req.query.word;
//     const url = `httsp://localhost:3000/api/users?word=${encodeURIComponent(word)}`;
//     axios
//         .get(url)
//         .then(response => {
...
module.exports = async (req, res) => {
  ...
  await mongoose.connect(
    'mongodb+srv://211354:Rx1yppNoqR7ljL2S@cluster0.3wcc5i2.mongodb.net/FetchedDictionary?retryWrites=true&w=majority',
    {
  ...
  switch (method) {
    ...
    case 'POST':
      try {
        ...
        res.status(201).json({ message: 'Dictionary entry created successfully' });
      } catch (error) {
        res.status(500).json({ message: error.message });
      }
      break;
    case 'PUT':
      try {
        const word = params.word;
        const Dictionaries = await Dictionary.findOneAndUpdate({ word }, body);
        if (!Dictionaries) {
          return res.status(404).json('Word Not Found');
        }
        const updatedDictionaries = await Dictionary.find({ word });
